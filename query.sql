...
--> event page
SELECT events.id, events.name, events.category, events.description, events."date", users.username
FROM events, users
WHERE events.owner_id = users.id AND events.id = $event_id;

SELECT posts.description, posts.id, posts.image_path, posts.user_id
FROM posts, events
